...
const InfoTooltip: React.FC<{ content: React.ReactNode; align?: 'left' | 'center' | 'right' }> = ({ content, align = 'center' }) => {
  ...
  return (
    <div className="relative inline-flex items-center ml-2 align-middle z-30">
      <button
        type="button"
        className="text-slate-300 dark:text-slate-600 hover:text-primary-500 dark:hover:text-primary-400 transition-colors focus:outline-none"
      ...
      <div 
        className={`
          absolute bottom-full mb-3 w-72 p-4 bg-slate-800/95 dark:bg-slate-700/95 backdrop-blur-sm text-slate-100 text-xs leading-relaxed rounded-2xl shadow-xl shadow-slate-900/20 transition-all duration-200 transform origin-bottom border border-slate-700 dark:border-slate-600
      ...
      >
        ...
        {/* Triangle pointer */}
         <div 
            className={`absolute -bottom-1.5 w-3 h-3 bg-slate-800/95 dark:bg-slate-700/95 border-r border-b border-slate-700 dark:border-slate-600 transform rotate-45 
...
const ScoreGauge: React.FC<{ score: number; isDark: boolean }> = ({ score, isDark }) => {
  const data = [
    { name: 'Accuracy', value: score, fill: score >= 8 ? '#10b981' : score >= 5 ? '#f59e0b' : '#ef4444' },
    { name: 'Max', value: 10, fill: isDark ? '#1e293b' : '#f1f5f9' },
  ];
  ...
  return (
    <div className="relative h-56 w-full flex items-center justify-center -mt-4">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mt-2">
        ...
        <span className="text-slate-400 dark:text-slate-500 text-xs font-bold uppercase tracking-widest mt-2">Trust Score</span>
      </div>
    </div>
  );
};
...
const ClaimCard: React.FC<{ claim: FactCheckClaim }> = ({ claim }) => {
  const colors = {
    True: 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20',
    False: 'bg-red-100 dark:bg-red-500/10 text-red-800 dark:text-red-400 border-red-200 dark:border-red-500/20',
    Misleading: 'bg-orange-100 dark:bg-orange-500/10 text-orange-800 dark:text-orange-400 border-orange-200 dark:border-orange-500/20',
    Unverified: 'bg-slate-100 dark:bg-slate-700/30 text-slate-800 dark:text-slate-300 border-slate-200 dark:border-slate-700',
    Mixed: 'bg-yellow-100 dark:bg-yellow-500/10 text-yellow-800 dark:text-yellow-400 border-yellow-200 dark:border-yellow-500/20',
  };

  const badgeColor = colors[claim.verdict] || colors.Unverified;

  return (
    <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-lg dark:shadow-slate-900/40 transition-all duration-300 group z-10 relative">
      <div className="flex justify-between items-start gap-4 mb-3">
        <h4 className="font-bold text-slate-800 dark:text-slate-200 text-base leading-snug group-hover:text-primary-700 dark:group-hover:text-primary-400 transition-colors">{claim.claim}</h4>
        <span className={`px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap border ${badgeColor}`}>
          {claim.verdict}
        </span>
      </div>
      <p className="text-slate-600 dark:text-slate-400 text-sm mb-4 leading-relaxed">{claim.explanation}</p>
      <div className="flex items-center gap-3">
         <div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden flex-1">
            <div 
         ...
         <div className="flex items-center">
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 whitespace-nowrap">Conf: {claim.confidenceScore}/10</span>
            <InfoTooltip 
                align="right"
                content={
                    <div>
...
export const Dashboard: React.FC<DashboardProps> = ({ result, chatSession, isDark }) => {
  return (
    <div className="space-y-8 animate-fade-in-up">
      {/* 1. Top Row: Scores & Summary */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {/* Score Card */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl shadow-slate-200/40 dark:shadow-slate-950/40 border border-slate-100 dark:border-slate-800 col-span-1 relative transition-colors">
            ...
            <div className="relative z-10 flex items-center justify-between">
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">Factual Accuracy</h3>
                <InfoTooltip 
                    align="right"
                    content={
                        <div>
        ...
        {/* Summary Card */}
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl shadow-slate-200/40 dark:shadow-slate-950/40 border border-slate-100 dark:border-slate-800 col-span-1 md:col-span-2 flex flex-col relative overflow-hidden transition-colors">
           {/* Decorative Top Line */}
           <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-primary-400 to-accent-500"></div>
          
          <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-6 relative z-10 flex items-center gap-2">
            Executive Analysis
          </h3>
          
          <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-lg relative z-10 mb-8 font-medium">
            {result.summary}
          </p>
          
          {/* New Scannable Key Takeaways Section */}
          {result.keyTakeaways && result.keyTakeaways.length > 0 && (
            <div className="relative z-10 mb-8 bg-surface-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 shadow-inner">
                <h4 className="text-xs font-extrabold text-primary-700 dark:text-primary-400 uppercase tracking-widest mb-4 flex items-center gap-2">
          ...
          {result.sources.length > 0 && (
            <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800 relative z-10">
              <h4 className="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {result.sources.slice(0, 4).map((source, idx) => (
                  <li key={idx}>
                    <a href={source.uri} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                       <span className="bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 p-1.5 rounded-md group-hover:bg-primary-100 dark:group-hover:bg-primary-900/50 transition-colors">
                         <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                         </svg>
                       </span>
                       <span className="text-sm text-slate-600 dark:text-slate-400 group-hover:text-primary-700 dark:group-hover:text-primary-300 font-medium truncate transition-colors">
                        {source.title}
                      </span>
                    </a>
                  </li>
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl shadow-slate-200/40 dark:shadow-slate-950/40 border border-slate-100 dark:border-slate-800 h-full transition-colors">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <span className={`px-3 py-1.5 rounded-lg text-sm font-bold border border-transparent ${
                            ...
                            result.commentAnalysis.overallSentiment === 'Negative' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800' :
                            'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700'
                        }`}>
                            {result.commentAnalysis.overallSentiment}
                        </span>
                    </div>
                    
                    <p className="text-slate-700 dark:text-slate-300 text-base mb-8 leading-relaxed font-medium">{result.commentAnalysis.summary}</p>

                    <div className="grid grid-cols-2 gap-4 mb-8">
                        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 relative">
                            <div className="flex items-center gap-1 mb-2">
                                <span className="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wide">Bot Probability</span>
                                <InfoTooltip 
                                    align="left"
                                    content={
                                        <div>
                            ...
                            <div className="flex items-end gap-3">
                                <span className="text-3xl font-extrabold text-slate-800 dark:text-slate-100">{result.commentAnalysis.botProbabilityScore}</span>
                                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-1 mb-2">
                                <span className="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wide">Logical Fallacies</span>
                                <InfoTooltip 
                                    align="right"
                                    content={
                                        <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {result.commentAnalysis.logicalFallacies.length > 0 ? (
                                    result.commentAnalysis.logicalFallacies.slice(0,3).map((f, i) => (
                                        <span key={i} className="text-xs font-semibold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-2 py-1 rounded-md">{f}</span>
                                    ))
                                ) : (
                                    <span className="text-sm font-medium text-slate-500 dark:text-slate-500 italic">None detected</span>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Dominant Emotions</span>
                        <div className="flex flex-wrap gap-2 mt-3">
                            {result.commentAnalysis.dominantEmotions.map((emotion, idx) => (
                                <span key={idx} className="bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 border border-purple-100 dark:border-purple-800 px-3 py-1.5 rounded-full text-sm font-semibold">
                                    {emotion}
                                </span>
                            ))}
                        </div>
